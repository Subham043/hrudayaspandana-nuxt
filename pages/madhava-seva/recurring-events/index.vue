...
<script>
export default {
    name: "MadhavaRecurringEventsPage",
    ...
    methods: {
        async getTableData(page=0,filter='madhava-seva',status=2) {
        ...
        handlePageChnage(){
            this.currentPage = this.$route.query.page ? Number(this.$route.query.page) : 1;
            this.getTableData(this.$route.query.page ? Number(this.$route.query.page) : 1, this.$route.query.filter ? this.$route.query.filter : 'madhava-seva', this.$route.query.status ? this.$route.query.status : 2);
            if(process.client){
                this.$scrollTo('#__nuxt', 0, {force: true})
            }
